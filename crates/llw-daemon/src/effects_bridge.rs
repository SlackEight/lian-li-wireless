...
pub fn geometry_of(kind: DeviceKind, fan_count: u8) -> Option<Geometry> {
    ...
    Some(Geometry::Fans { fan_count, leds_per_fan: lpf })
}
...
///
/// `frame_budget` is the number of frames to render (controlled by Task 8's probe).
pub fn compile(
    spec: &EffectSpec,
    rec: &DeviceRecord,
    frame_budget: u16,
...
mod tests {
    ...
    fn sl_inf_3fan_geometry() {
        ...
        assert_eq!(geom, Geometry::Fans { fan_count: 3, leds_per_fan: 44 });
